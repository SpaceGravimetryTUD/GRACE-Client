  ...
  if cfg == {}:
    ...
    for fld1 in list(cfg_tmp.keys()):
       if fld1 not in list(cfg.keys()):
         warnings.warn("First level field \"" + fld1 + "\" not found in configuration file. Query will be carried out with field value flagged in the cmmand line or assumed to be missing. If needed, add it to \"" + cfg_path + "\" with the following second level fields: \"" + "\", \"".join(list(cfg_tmp[fld1].keys())) + "\".", UserWarning)
       else:
         for fld2 in list(cfg_tmp[fld1].keys()):
           if fld2 not in list(cfg[fld1].keys()):
             warnings.warn("Second level field \"" + ".".join([fld1,fld2]) + "\" not found in configuration file. Query will be carried out with field value flagged in the cmmand line or assumed to be missing.", UserWarning)
           else:
             pass
